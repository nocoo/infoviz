...
define(function(require, exports, module) {
    exports.draw_linechart = function(paper, chart_area, data, overwrite_options, callback, that) {
        ...
        var v_label_value_unit = (v_max - v_min) / (options['linechart']['vertical-label-count'] - 1); // May not be accurate.
        var v_label_unit2 = (v_start - chart_area['top-left'][1] - options['linechart']['padding-top']) / (options['linechart']['vertical-label-count-right'] - 1);
        var v_label_value_unit2 = (v_max2 - v_min2) / (options['linechart']['vertical-label-count-right'] - 1); // May not be accurate.
        ...
        for (i = 0; i < options['linechart']['vertical-label-count']; ++i) {
            ...
            // vertical label.
            paper.text(x - options['linechart']['vertical-bar-width'], y, l_string).attr({
                'text-anchor': 'end',
        ...
        if (options['grid']['enable-right-axis']) {
            ...
            y = v_start;
            v_value = v_min2;
            for (i = 0; i < options['linechart']['vertical-label-count-right']; ++i) {
                cache.push('M' + x + ',' + y + 'L' + (chart_area['top-right'][0] + options['linechart']['vertical-bar-width-right']) + ',' + y);
                ...
                // vertical label.
                paper.text(x + 2 * options['linechart']['vertical-bar-width-right'], y, l_string).attr({
                    'text-anchor': 'start',
                    'fill': options['grid']['vertical-label-color'],
                    'font-size': options['grid']['vertical-label-size']
                }).translate(0.5, 0.5);
