...
define(function(require, exports, module) {
    require.async(['./infoviz.core'], function(core) {
        ...
        exports.draw_streamchart = function(paper, chart_area, data, overwrite_options, callback, that) {
            ...
            var v_label_unit = v_height / (options['streamchart']['vertical-label-count'] - 1);
            var v_label_value_unit = 1 / (options['streamchart']['vertical-label-count'] - 1);
            v_label_value_unit = Math.round(v_label_value_unit * Math.pow(10, 2)) / Math.pow(10, 2);

            cache = [];
            x = chart_area['top-left'][0] - options['streamchart']['vertical-bar-width'];
            y = v_start;
            var v_value = 0;

            for (i = 0; i < options['streamchart']['vertical-label-count']; ++i) {
                ...
                paper.text(x - options['streamchart']['vertical-bar-width'], y, v_value).attr({
                    'text-anchor': 'end',
